...
function fetchPlatformToken() {
    ...
    if (!$response || $httpcode !== 200) {
        return ['error' => 'Failed to fetch platform page'];
    }
...
function fetchM3U8url() {
    ...
    if (is_array($platformToken)) return $platformToken;
    ...
    curl_setopt_array($ch, [
        CURLOPT_URL =>
            ...
            '&app_version=4.24.0&user_type=guest&check_parental_control=false',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST => true,
        CURLOPT_HTTPHEADER => [
    ...
    if (!isset($data['keyOsDetails']['video_token'])) {
        return ['error' => 'Video token not found'];
    }
...
function generateCookieZee5($userAgent) {
    ...
    if (is_array($m3u8Url)) return $m3u8Url;
